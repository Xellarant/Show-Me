...
<?php
  ...
  VerifyEmployeesTable($conn, DB_DATABASE); 
  ...
  if (strlen($employee_name) || strlen($employee_address)) {
    AddEmployee($conn, $employee_name, $employee_address);
  }
?>
...
function AddEmployee($conn, $name, $address) {
   $n = mysqli_real_escape_string($conn, $name);
   $a = mysqli_real_escape_string($conn, $address);

   $query = "INSERT INTO `Employees` (`Name`, `Address`) VALUES ('$n', '$a');";

   if(!mysqli_query($conn, $query)) echo("<p>Error adding employee data.</p>");
}

/* Check whether the table exists and, if not, create it. */
function VerifyEmployeesTable($conn, $dbName) {
  if(!TableExists("Employees", $conn, $dbName)) 
  { 
